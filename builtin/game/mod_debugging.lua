...
minetest.log("action", 'Mod debugging enabled')
...
function mod.assert(recipe, _name, output)
	...
	if name == nil then
		minetest.log('error', 'nil in recipe for '..mod.strip_name(output))
		print(recipe.from)
		return
	end
	...
	if minetest.registered_items[name] == nil and not group_exists(name) then
		minetest.log( 'error', 'missing item '..name.." in recipe for "..mod.strip_name(output) )
		print(recipe.from)
	end
end
