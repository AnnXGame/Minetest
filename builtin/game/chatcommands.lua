...
if minetest.setting_getbool("mod_profiling") then
	local tracefct = profiling_print_log
	profiling_print_log = nil
	minetest.register_chatcommand("save_mod_profile",
			{
				params      = "",
...
minetest.register_chatcommand("setpassword", {
	...
	func = function(name, param)
		...
		if not raw_password then
			...
			minetest.set_player_password(toname,
					minetest.get_password_hash(toname,
							raw_password))
			actstr = "set"
		end
		if toname ~= name then
			minetest.chat_send_player(toname, "Your password was "
					.. actstr .. " by " .. name)
		end
...
minetest.register_chatcommand("deleteblocks", {
	...
	func = function(name, param)
		...
		if param == "here" then
			...
			p1 = minetest.string_to_pos(pos1 .. ")")
			p2 = minetest.string_to_pos("(" .. pos2)
		...
		if minetest.delete_area(p1, p2) then
			return true, "Successfully cleared area ranging from " ..
				minetest.pos_to_string(p1, 1) .. " to " .. minetest.pos_to_string(p2, 1)
		else
...
minetest.register_chatcommand("spawnentity", {
	...
	func = function(name, param)
		...
		minetest.log("action", ("/spawnentity invoked, entityname=%q")
				:format(entityname))
		local player = minetest.get_player_by_name(name)
		if player == nil then
...
minetest.register_chatcommand("time", {
	...
	func = function(name, param)
		...
		if not hour then
			...
			minetest.set_timeofday((new_time % 24000) / 24000)
			minetest.log("action", name .. " sets time to " .. new_time)
		...
		minetest.set_timeofday((hour * 60 + minute) / 1440)
		minetest.log("action", name .. " sets time to " .. hour .. ":" .. minute)
		return true, "Time of day changed."
	end,
})
...
minetest.register_chatcommand("clearobjects", {
	...
	func = function(name, param)
		...
		minetest.chat_send_all("Clearing all objects.  This may take long."
...
minetest.register_chatcommand( "stat", {
	...
	func = function(name, param)
		...
		local formspec = minetest.stat_formspec(param)
		minetest.show_formspec(name, 'stat', formspec)
	end
})
