...
local function build_callback(log_id, fct)
	return function( toregister )
		...
		fct(function(...)
			...
			local delta = minetest.get_us_time() - starttime
...
function profiling_print_log(cmd, filter)
	...
	minetest.log("action", "Values below show times/percentages per server step.")
	minetest.log("action", "Following suffixes are used for entities:")
	minetest.log("action", "\t#oa := on_activate, #os := on_step, #op := on_punch, #or := on_rightclick, #gs := get_staticdata")
	minetest.log("action",
	...
	for modname,statistics in pairs(mod_statistics.stats) do
		if modname ~= "total" then
			...
			if (filter == "") or (modname == filter) then
				...
				if minetest.setting_getbool("detailed_profiling") then
...
local function initialize_profiling()
	minetest.log("action", "Initialize tracing")
	
	mod_statistics.entity_callbacks = {}
	
	-- first register our own globalstep handler
	minetest.register_globalstep(mod_statistics.update_statistics)
	
	local rp_register_entity = minetest.register_entity
	...
	local rp_register_abm = minetest.register_abm
	...
	minetest.log("action", "Mod profiling initialized")
end
