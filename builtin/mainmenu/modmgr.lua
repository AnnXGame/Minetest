...
function modmgr.get_worldconfig(worldpath)
    ...
    for key,value in pairs(worldfile:to_table()) do
        if key == "gameid" then
            ...
            worldconfig.global_mods[key] = minetest.is_yes(value)
        end
    end
...
function modmgr.installmod(modfilename,basename)
    ...
    if basefolder.type == "modpack" then
        ...
        if clean_path ~= nil then
            local targetpath = minetest.get_modpath() .. DIR_DELIM .. clean_path
    ...
    if basefolder.type == "mod" then
        ...
        if targetfolder ~= nil and modmgr.isValidModname(targetfolder) then
            local targetpath = minetest.get_modpath() .. DIR_DELIM .. targetfolder
            minetest.copy_dir(basefolder.path,targetpath)
        else
            gamedata.errormessage = fgettext("Install Mod: unable to find real modname for: $1", modfilename)
        end
    end

    minetest.delete_dir(modpath)
...
function modmgr.preparemodlist(data)
    ...
    for key,value in pairs(worldfile:to_table()) do
        if key:sub(1, 9) == "load_mod_" then
            ...
            if element ~= nil then
                element.enabled = minetest.is_yes(value)
            else
